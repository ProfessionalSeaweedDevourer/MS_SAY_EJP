...
# 사용자가 이기는 3가지 경우만 명시 (플레이어, 컴퓨터)
winningResult = [(1, 3), (2, 1), (3, 2)]
...
def rspGame_tuple(player, computer): # 튜플 검사를 활용한 승패 판정
    if player == computer:
        return 0  # 0: 무승부
    elif (player, computer) in winningResult:  # 미리 정의한 '유저 승리 경우'에 해당하면 승리
        return 1  # 1: 승
    else:
        return 2  # 2: 패
...
while True:
    ...
    # 승패 판정
    result = rspGame_tuple(user_input, cpu_input)
